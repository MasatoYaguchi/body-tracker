...
import { err, ok, type Result, toAuthError } from "../domain/result";
...
export async function handleAuthCallback(
	...
	if (state !== "login")
		return err(toAuthError("STATE_MISMATCH", "state mismatch"));
	const verifier = sessionStorage.getItem(PKCE_VERIFIER_KEY);
	if (!verifier)
		return err(
			toAuthError("PKCE_VERIFIER_MISSING", "verifier missing (restart login)"),
		);
	try {
		const redirectUri = `${window.location.origin}/auth/callback`;
		console.log("authCodeFlow: exchanging code", {
		...
		console.log("authCodeFlow: exchangeAuthorizationCode returned", {
			user: user?.email,
		});
		...
		return err(
			toAuthError(
				...
				(e as Error).message || "code exchange failed",
				e,
			),
		);
	}
