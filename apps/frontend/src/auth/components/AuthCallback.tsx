...
export function AuthCallback() {
	...
	useEffect(() => {
		let cancelled = false;
		// Debug: mark that the callback component mounted and show the URL
		console.log("AuthCallback mounted", window.location.href);
		(async () => {
			try {
				console.log("AuthCallback: calling handleAuthCallback");
				const result = await handleAuthCallback(window.location.href);
				console.log("AuthCallback: handleAuthCallback returned", result);
				if (cancelled) return;
				if (!result.ok) {
	...
	return (
		<div className="min-h-screen flex items-center justify-center bg-gray-50">
			<div className="bg-white p-6 rounded shadow w-full max-w-sm text-center space-y-4">
				...
				<p
					className={isError ? "text-red-600 text-sm" : "text-gray-700 text-sm"}
				>
					{message}
				</p>
				{!isError && (
