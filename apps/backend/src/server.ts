...
app.get('/api/records', async (c) => {
  try {
    const allRecords = await db.select().from(bodyRecords).orderBy(desc(bodyRecords.recordedDate));
...
app.delete('/api/records/:id', async (c) => {
  try {
    ...
    const [deletedRecord] = await db.delete(bodyRecords).where(eq(bodyRecords.id, id)).returning();
